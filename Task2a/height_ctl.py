...
class HeightControl:
    ...
    def __init__(self):
        ...
        #---------------- height ONLY
        # self.pid_vz.set_kp(80)    
        # self.pid_vz.set_ki(0)     
        # self.pid_vz.set_kd(1.414)     
        
        # self.pid_z.set_kp(1.1)      
        # self.pid_z.set_ki(0.125)      
        # self.pid_z.set_kd(0)      
        # --------------------------------------------

        # height + altitude BOTH
        # vz - inner loop
        self.pid_vz.set_kp(100)     
        self.pid_vz.set_ki(100)      
        self.pid_vz.set_kd(0)     
        
        # z - outer loop
        self.pid_z.set_kp(0.6)      
        self.pid_z.set_ki(0)      
        self.pid_z.set_kd(0.4)      
        ...
        self.ros_rate = rospy.Rate(20)   #20 h
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            ########################################################
            ########################################################
            # 1. Implement cascade PID control here.
            ...
            # Hover thrust (in terms of motor velocity)
            hover_velocity = math.sqrt((self.mass * self.gravity)/(4 * self.bf))

            # Outer loop - height control
            vz_ref = self.pid_z.compute(self.z_sp, self.z_mv)
            
            # Inner loop - velocity control
            delta_omega = self.pid_vz.compute(vz_ref, self.vz_mv)

            # total thrust = hover thrust + delta omega
            self.mot_speed = hover_velocity + delta_omega
